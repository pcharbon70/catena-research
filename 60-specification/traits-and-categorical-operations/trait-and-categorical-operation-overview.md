...
This chapter and its six sibling chapters are the normative Catena 0.4 trait
slice. They extend the [0.1 type system](../type-system/README.md), the
[0.2 data model](../data-and-patterns/README.md), and the
[0.3 clause-condition boundary](../clause-conditions/README.md). `MUST`,
`MUST NOT`, `SHOULD`, and `MAY` state conformance requirements.

The immutable implementation and conformance identity are recorded in
...
```mermaid
flowchart LR
    A[JSON AST 0.4] --> K[Kind and declaration checking]
...
0.4 adds no Rust, Python, Core Erlang emitter, BEAM assembler, runtime
...
Version 0.4 does not add law-directed rewrites, runtime dictionary identity,
