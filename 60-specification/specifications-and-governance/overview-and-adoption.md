...
This chapter and its five siblings are the normative Catena 0.6 assurance
slice. They extend the normative 0.1 through 0.5 language slices without
...
satisfied by the authorized immutable compiler commit and reproducible evidence
recorded in the
...
two explicit boundaries. A package with no 0.6 declarations or manifest uses
the ordinary 0.1–0.5 pipeline, emits no assurance manifest, loads no trust
root, and produces the same BEAM bytes it produced before 0.6. Declaring 0.6
...
Version 0.6 defines exactly three canonical JSON artifacts:

1. `catena-trust-root` version `0.6` identifies offline Ed25519 principals,
   roles, thresholds, delegations, recovery authority, and root history;
2. `catena-governance-bundle` version `0.6` contains policies, evidence,
   approvals, and lifecycle transitions for one package; and
3. `catena-assurance-manifest` version `0.6` binds the validated semantic graph
   and governance result to exact emitted artifacts.

The package manifest remains the build entry point. A 0.6 manifest names its
...
The bounded 0.6 slice provides:
...
Version 0.6 does not freeze public parser punctuation, runtime contracts or
...
Format. Version 0.6 does not authorize Rust or Python compiler components,
