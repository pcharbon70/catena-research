...
- [Haskell 2010 Language Report](marlow-2010-haskell-language-report.md) —
  ...
  with defaulting and undefined fixed-precision exceptional conditions, and module headers paired with filename conventions that default missing headers to `Main`. Its section 1.4 fixes six name kinds in two spelling classes with qualification as the ambiguity escape, and its fixity and module sections anchor the operator and file-binding comparisons. Chapter 5 fixes explicit import lists with empty qualified-only forms, always-available qualification, mention-time clash errors, and public-by-default exports with `hiding`, aliases, and module re-exports. Sections 5.1 and 5.7 admit mutually recursive modules while conceding the separate-compilation price: explicit signatures on exported values and group-level compilation units. Sections 5.6–5.6.2 fix the implicit-unless-explicit Prelude with empty-import exclusion and `hiding` disambiguation.
