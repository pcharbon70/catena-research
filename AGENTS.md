...
Normative C010 uses `0.1.8`, normative C013 uses `0.1.9` for source text,
normative C014 uses `0.1.10` for identifiers and standalone qualified names,
...
`0.1.50`. Normative C084 uses `0.1.52` for owned tasks and managed relationships. Normative C088 uses `0.1.53` for cancellation and time. Normative C103 uses `0.1.54` for outcome contracts. Normative C087 uses `0.1.55` for local protocol contracts. Normative C089 uses `0.1.56` for typed supervision. Normative C099 uses `0.1.57` for OTP compatibility. Normative C093 uses `0.1.58` for value boundaries. Normative C094 uses `0.1.59` for calling conventions. Normative C095 uses `0.1.60` for the Erlang type boundary. The normative P096 foreign-adapter milestone uses `0.1.61`. Normative C097 uses `0.1.62` for native value roles. Normative C098 uses `0.1.63` for native services. Normative C100 uses `0.1.64` for debugging metadata. Normative C102 uses `0.1.65` for collection protocols. Normative C104 uses `0.1.66` for the text and binary model. Normative C105 uses `0.1.67` for the numeric library. Normative C106 uses `0.1.68` for environmental effects. Normative C101 uses `0.1.69` for the minimum prelude. Normative C126 uses `0.1.70` for the trusted computing base. Normative C127 uses `0.1.71` for trusted obligation policy. Normative C131 uses `0.1.72` for secret capabilities. Normative C128 uses `0.1.73` for reproducible builds. Normative C130 uses `0.1.74` for signed package registry and immutable acquisition. Normative C129 uses `0.1.75` for aggregate budgets and runtime admission. Normative C091 uses `0.1.76` for typed authenticated distribution. Normative C085 uses `0.1.77` for integrated message semantics. Normative C090 uses `0.1.78` for scheduler observability. Normative C092 uses `0.1.79` for checked hot code upgrade. Normative C116 uses `0.1.80` for long-term evolution. Normative C121 uses `0.1.81` for the build system and package manager. Normative C136 uses `0.1.82` for the bounded compatibility suite. Normative C133 uses `0.1.83` for integrated reference observations. Normative C122 uses `0.1.84` for bounded testing tools. Normative C134 uses `0.1.85` for systematic differential testing. Normative C135 uses `0.1.86` for checked optimizer validity. Normative P108 uses `0.1.87` for standard stability and performance. Normative G138 uses `0.1.88` for the supported-host performance envelope. Normative C124 uses `0.1.89` for debugging and observability. Normative G139 uses `0.1.90` for release-readiness evidence and claim classes. Normative P117 uses `0.1.91` for the semantic diagnostic contract. Normative P119 uses `0.1.92` for the retained-input documentation tool. Normative P125 uses `0.1.93` for transactional retained-JSON migrations. Normative G118 uses `0.1.94` for the syntax-independent formatter foundation. Normative G120 uses `0.1.95` for owned retained-input interactive sessions. Normative G123 uses `0.1.96` for the immutable retained-input language service. Normative G137 uses `0.1.97` for the observed usability gate and pre-study package. Normative G141 uses `0.1.98` for the staged self-hosting milestone and blocked preflight. The normative C034 opt-in totality validity amendment uses `0.1.99`. The next unused semantic patch is `0.1.100` unless an approved versioning policy explicitly
...
- Use portable inline Markdown links for citations. Do not use footnote-reference
  syntax such as `[^1]` and `[^1]:`, because supported archive readers may show
  those markers as literal text. A substantial synthesis may use compact linked
  numbers such as `[[1]](../30-sources/example.md)` near claims, paired with an
  ordinary numbered `## Sources` bibliography containing the primary link and
  local source-note link. Descriptive author–year links are preferable when
  they read naturally in the sentence.
...
- rendered local Markdown file and heading-fragment links, excluding literal
  fenced code and inline code spans while retaining code-formatted link labels;
