...
- [Approachable Catena Language Design](approachable-catena-language-design.md)
  — connects the behavior-first vocabulary, validation inquiry, diagnostic
  research, and the formal models the public language must preserve.
...
- [API and ABI Compatibility](api-and-abi-compatibility.md) — routes
  ...
  tiered-compat precedent, and the proposed strict interface diff
  matrix with minor-as-breaking under 0.x and the claim validator.
...
- [Whitespace, Layout, and Line Continuation](whitespace-layout-and-line-continuation.md)
  — routes through source-text foundations, comparative layout designs,
  indentation invariance, separators, continuation capabilities, C015, and
  remaining lexical owners.
...
Index every direct map, but keep each map's own route selective. Update the
home map only for major active inquiries, topic maps, and developed synthesis.
